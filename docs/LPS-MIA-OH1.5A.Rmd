...
load("04132021_LPS orgs_base.RData")
...
```{r pre-processing, echo=TRUE, fig.height = 4, fig.width = 4, fig.align = "center"}
#Subsetting a df to samples of interest (accute)
subset.df <- gfilt.df[colnames(gfilt.df) %in% rownames(filt.meta[filt.meta$Batch %in% 2 & filt.meta$`Cell line` %in% c("OH1.5") & filt.meta$`Accute/chronic` %in% c("Accute"),])]
...
```{r norm.qc1, echo=TRUE, fig.height = 5, fig.width = 5, fig.align = "center"}
...
```{r norm.qc3, echo=TRUE, fig.height = 5, fig.width = 5, fig.align = "center"}
#PCA
#550x450
...
```{r QC.post, echo=TRUE, fig.height = 5, fig.width = 5, fig.align = "center"}
...
```{r QC.post2, echo=TRUE, fig.height = 4, fig.width = 4, fig.align = "center"}
...
##Plotting results
(6) Plotting results
```{r plots.5, echo=TRUE, fig.height = 6, fig.width = 8, fig.align = "center"}
#Volcano plot
#700x750
...
##Boxplots of DEGs
(7) Boxplots of DEGs
```{r plots.7, echo=TRUE, fig.height = 8, fig.width = 12, fig.align = "center"}
#Boxplots of DEGs
rownames(sub) <- gsub('\\.', '_', rownames(sub))
rownames(sub) <- gsub('\\-', '_', rownames(sub))
box <- data.frame(scale(t(sub)), Stimulation=annot_df[order(fact.to.sort, decreasing=F),][1])
