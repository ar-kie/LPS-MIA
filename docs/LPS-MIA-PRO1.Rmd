...
subset.df <- gfilt.df[colnames(gfilt.df) %in% rownames(filt.meta[filt.meta$`Cell line` %in% c("PRO1") & filt.meta$`Accute/chronic` %in% c("Accute"),])]
...
dds.complex <- DESeqDataSetFromMatrix(countData = as.matrix(subset.df),
                                      ...
                                      design = ~ Log.library.size. + Scaled.percent..MT + Percent..ERCC + Stimulation)
...
batch.rem <- removeBatchEffect(as.matrix(assay(vsd.complex)), 
                               covariates=as.matrix(cbind(covs_forremBatch$`Log(library size)`,
                                                          covs_forremBatch$`Percent. ERCC`, covs_forremBatch$`Percent. MT`
                                                          )),
...
But here, take a look either way:
## LPS response genes

(5) LPS response and microglia genes
...
LPS.sub <- batch.rem[rownames(batch.rem) %in% c(LPS.genes),]
