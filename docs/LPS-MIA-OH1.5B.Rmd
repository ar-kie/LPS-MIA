...
subset.df <- gfilt.df[colnames(gfilt.df) %in% rownames(filt.meta[filt.meta$Batch %in% 3 & filt.meta$`Cell line` %in% c("OH1.5") & filt.meta$`Accute/chronic` %in% c("Accute"),])]
...
dds.complex <- DESeqDataSetFromMatrix(countData = as.matrix(subset.df),
                                      ...
                                      design = ~ Log.library.size. + Percent..ERCC + Scaled.percent..MT + Stimulation)
...
batch.rem <- removeBatchEffect(as.matrix(assay(vsd.complex)), 
                               covariates=as.matrix(cbind(covs_forremBatch$`Log(library size)`,covs_forremBatch$`Percent. MT`,
...
createDT(data.frame(res.complex), "OH1.5B", scrollY=1000)
...
downreg.genes <- rownames(res.complex[res.complex$log2FoldChange < -1 & res.complex$padj < 0.1,])
upreg.genes <- rownames(res.complex[res.complex$log2FoldChange > 1 & res.complex$padj < 0.1,])
